...
function PairPage(props) {
  ...
  return (
    <AppShell>
      <Head>
        <title>
          {pair.token0.symbol}-{pair.token1.symbol} | Duly Analytics
        </title>
      </Head>
      <PageHeader>
        <Box display="flex" alignItems="center" className={classes.pageHeader}>
          ...
          <Box display="flex" alignItems="center" className={classes.links}>
            <Link
              href={`https://duly.com/add/${pair.token0.id}/${pair.token1.id}`}
              target="_blank"
              variant="body1"
            ...
            <Link
              href={`https://duly.com/swap?inputCurrency=${pair.token0.id}&outputCurrency=${pair.token1.id}`}
              target="_blank"
              variant="body1"
            >
              Trade
